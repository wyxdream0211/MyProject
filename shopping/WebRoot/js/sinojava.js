		...
		function myAjax(){
			...
			if(xmlHttpRequest != null){
				if(document.reg.username.value == "") {
					alert("请输入用户名!");
					document.reg.username.focus(); //设置焦点
					return false;
				}
				var v1 = document.reg.username.value;
				
				//采用POST方式提交时，提交的数据作为消息体的一部分发送给服务器端
				//1.建立连接
		...
		function ajaxCallback(){
			if(xmlHttpRequest.readyState == 4){
				if(xmlHttpRequest.status == 200){
					...
					if(text == 'true'){
						document.getElementById("div1").innerText = "用户名"+v1+"可以使用";
						document.getElementById("div1").color = "green";
					}else if(text == 'false'){
						document.getElementById("div1").innerText = "用户名"+v1+"已被注册，请使用其他用户名";
